...
class TableArticle extends Table {
	...
	public function __construct()
	{
		...
		$this->_selectOne = "SELECT *, DATE_FORMAT(dateCreation, '%d %M %Y') AS dateInscriptionFormatee FROM article WHERE id = :id ";
		$this->_search = "SELECT *, DATE_FORMAT(dateCreation, '%d %M %Y') AS dateInscriptionFormatee FROM article WHERE nomRubrique = :nomRubrique AND (titre LIKE :test OR infos LIKE :test OR emailUtilisateur LIKE :test) ORDER BY dateCreation DESC";
	}
